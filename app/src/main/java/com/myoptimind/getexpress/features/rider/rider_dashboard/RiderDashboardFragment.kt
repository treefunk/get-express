...
import kotlinx.android.synthetic.main.fragment_rider_dashboard.*
...
@AndroidEntryPoint
class RiderDashboardFragment: LogoOnlyFragment() {


    var job: Job? = null

    private val viewModel by viewModels<RiderDashboardViewModel>()
    ...
    private fun refresh() = lifecycleScope.launchWhenResumed {
        for(i in REFRESH_INTERVAL_SECOND downTo 1){
            Timber.v("hashcode - ${this.hashCode()} refreshing in $i..")
            delay(1000)
        }
//        adapter.customerRequests = ArrayList()
...
//        job.start()
    ...
    private fun updateUserLabels() {
        val user = appSharedPref.getRiderFromPrefs()
        Glide.with(requireContext())
                .load(user.profilePicture)
                .into(iv_profile_picture)

        tv_profile_full_name.text = "Welcome, ${user.fullName}"
        tv_caption.text = "${user.activeVehicle.vehicleModel} | ${user.activeVehicle.plateNumber}"
    }
        ...
        adapter = CustomerRequestAdapter(ArrayList(), object: CustomerRequestAdapter.CustomerRequestListener{
            override fun onSelectItem(customerRequest: CustomerRequest) {
                findNavController().navigate(
                        RiderDashboardFragmentDirections.actionRiderDashboardFragmentToSelectedCustomerRequestFragment(customerRequest.cartId)
                )
            }
            ...
            override fun onPressButton(accepted: Boolean, customerRequest: CustomerRequest) {
                MaterialAlertDialogBuilder(requireContext())
                        ...
                        .setPositiveButton("ACCEPT") { _, _ ->
                            findNavController().navigate(
                                    RiderDashboardFragmentDirections.actionRiderDashboardFragmentToSelectedCustomerRequestFragment(customerRequest.cartId,accepted)
                            )
                        }
                        .show()
                ...
                is Result.Success -> {
                    if(result.data != null){
                        if(
                                ...
                                job!!.isCompleted // if request is not yet finish
                        ){
